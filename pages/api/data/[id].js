...
const { deleteRecordById } = require('../../../lib/deletionService');
...
export default async function handler(req, res) {
  ...
  const db = readDb();
  const deletion = await deleteRecordById(db, req.query.id, auth.user);
  if (deletion.error) return sendError(res, deletion.status, deletion.error);
  writeDb(db);
  return sendSuccess(res, { ok: true, deletedId: req.query.id, entity: 'data' });
}
